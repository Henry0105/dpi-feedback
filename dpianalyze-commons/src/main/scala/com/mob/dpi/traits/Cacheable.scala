...
trait Cacheable extends Watching {
  ...
  def partitionExists(name: String, partitions: Map[String, String]): Boolean = {
    val Array(dbName, tableName) = name.split("\\.")
    spark.sessionState.catalog.listPartitions(TableIdentifier(tableName, Some(dbName)), Some(partitions)).nonEmpty
  }
}
