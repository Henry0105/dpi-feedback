 ...
 * @Date: 2022/3/2 10:16
 */
public class APIToHiveOrc_HostDay {
    ...
    public static void main(String[] args) throws Exception {
        ...
        // 写入Hive表，格式【数据库.数据表】
        String hiveTable = args[2];


        List<ComparableList> rows = HttpUtils.getApiDataHostDay(key,schema);

        if(rows.size() == 0){
            logger.error("{}数据为空",key);
            System.exit(-1);
        }

        logger.info("{}数据拉取完成，条数{}",key,rows.size());
        ...
        logger.info("{}数据存入hive表{}",key,hiveTable);
